...
fn init_model() -> R<BertModel> {
    let repo = Repo::with_revision(MODEL_ID.parse()?, RepoType::Model, REVISION.parse()?);
    let (config_filename, weights_filename) = {
        let api = Api::new()?.repo(repo);
        (api.get("config.json")?, api.get("model.safetensors")?)
    };
...
impl BertSelfAttention {
    ...
    fn forward(&self, hidden_states: &Tensor) -> Result<Tensor> {
        ...
        let attention_probs = { candle_nn::ops::softmax(&attention_scores, candle::D::Minus1)? };
